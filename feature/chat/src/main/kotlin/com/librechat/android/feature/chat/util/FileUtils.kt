...
/** Draft key used for new chats that don't have a conversation ID yet. */
const val NEW_CHAT_DRAFT_KEY = "__new_chat__"
...
internal fun reEncodeImageIfNeeded(bytes: ByteArray, mimeType: String): ReEncodedImage? {
    ...
    return try {
        ...
        if (verifiedType != "image/png") {
            Timber.w(
                "reEncodeImageIfNeeded: re-encoded bytes don't match expected type image/png (got %s), using original",
                verifiedType,
            )
            return null
        }
        ...
        Timber.d(
            "reEncodeImageIfNeeded: re-encoded %s -> image/png (%d -> %d bytes)",
            mimeType, bytes.size, encodedBytes.size,
        )
        ReEncodedImage(bytes = encodedBytes, mimeType = "image/png")
    } catch (e: Exception) {
        Timber.w(e, "reEncodeImageIfNeeded: failed to re-encode %s, using original bytes", mimeType)
        null
    }
}
